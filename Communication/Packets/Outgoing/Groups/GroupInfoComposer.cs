...
namespace Quasar.Communication.Packets.Outgoing.Groups
{
    class GroupInfoComposer : ServerPacket
    {
            ...
            : base(ServerPacketHeader.GroupInfoMessageComposer)
        {
            ...
            base.WriteString((QuasarEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId) == null) ? "Quarto não encontrado.." : QuasarEnvironment.GetGame().GetRoomManager().GenerateRoomData(Group.RoomId).Name);    // room name

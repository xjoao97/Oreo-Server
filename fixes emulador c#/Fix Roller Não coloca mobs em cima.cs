...
using Oreo.HabboHotel.Items.Data.Toner;
            ...
            foreach (Item Item in Items.ToList())
            {
                ...
                if (Item.IsFloorItem)
                {
                    if (!_room.GetGameMap().ValidTile(Item.GetX, Item.GetY))
                    {
                        ...
                        GameClient Client = OreoServer.GetGame().GetClientManager().GetClientByUserID(Item.UserID);
                        if (Client != null)
                        {
        ...
        public bool SetFloorItem(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false)
        {
            ...
            if (newItem)
            {
                if (Item.IsWired)
                {
                    ...
                    {
                        Item[] floorItems = _room.GetRoomItemHandler().GetFloor.ToArray();
                        if (floorItems.Count() > 0)
                        {
                            for (int i = 0; i < floorItems.Count(); i++)
                            {
                                Item it = floorItems[i];
                                if (it.GetBaseItem().WiredType == WiredBoxType.EffectRegenerateMaps)
                                    return false;
                            }
                        }
                    }
            ...
            Item[] ItemsOnTile = GetFurniObjects(newX, newY).ToArray();
            ...
            if (newItem)
            {
                ...
                {
                    if (Session != null)
                        Session.SendNotification(OreoServer.GetGame().GetLanguageManager().TryGetValue("room.item.already_placed"));
                    _room.GetGameMap().RemoveFromMap(Item);
                    return true;
                }

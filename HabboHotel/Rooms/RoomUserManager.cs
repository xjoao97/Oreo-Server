        ...
        public void RemoveUserFromRoom(GameClient Session, Boolean NotifyClient, Boolean NotifyKick = false)
        {
            try
            {
                ...
                if (User != null)
                {
                    ...
                    //if (_room.QueueingUsers.Count > 0)
                    //{
                    //    foreach (Habbo user in _room.QueueingUsers)
                    //    {
                    //        if (_room.QueueingUsers.First().Id == user.Id)
                    //        {
                    //            user.PrepareRoom(_room.Id, "");
                    //        }
                    //        else
                    //        {
                    //            user.GetClient().SendMessage(new RoomQueueComposer(_room.QueueingUsers.IndexOf(user)-1));
                    //        }
                    //    }
                    //    _room.QueueingUsers.Remove(_room.QueueingUsers.First());
                    //}
                }
            }
            catch (Exception e)
                        ...
                        case InteractionType.PINATA:
                            {
                                if (User.IsWalking && Item.ExtraData.Length > 0)
                                {
                                    ...
                                    if (givenHits < 1 && User.CurrentEffect == 158)
                                    {
                                        ...
                                        if (givenHits == 1) QuasarEnvironment.GetGame().GetPinataManager().ReceiveCrackableReward(User, _room, Item);

                                        #region Achievements
                                        QuasarEnvironment.GetGame().GetAchievementManager().ProgressAchievement(User.GetClient(), "ACH_PinataWhacker", 1);

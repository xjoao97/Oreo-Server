...
namespace Quasar.HabboHotel.Support
{
    ...
    public class SupportTicket
    {
        ...
        public SupportTicket(int Id, int Score, int Type, int SenderId, int ReportedId, String Message, int RoomId, String RoomName, Double Timestamp, List<string> ReportedChats)
        {
            ...
            SenderName = QuasarEnvironment.GetGame().GetClientManager().GetNameById(SenderId);
            ReportedName = QuasarEnvironment.GetGame().GetClientManager().GetNameById(ReportedId);
            ModName = QuasarEnvironment.GetGame().GetClientManager().GetNameById(ModeratorId);
        }
        ...
        public void Pick(int pModeratorId, Boolean UpdateInDb)
        {
            ...
            ModName = QuasarEnvironment.GetHabboById(pModeratorId).Username;
            if (UpdateInDb)
            {
                using (IQueryAdapter dbClient = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.RunQuery("UPDATE moderation_tickets SET status = 'picked', moderator_id = " + pModeratorId + ", timestamp = '" + QuasarEnvironment.GetUnixTimestamp() + "' WHERE id = " + Id + "");
                }
            }
        }

...
namespace Emulator.HabboHotel.Items
{
    public class ItemFactory
    {
        ...
        public static List<Item> CreateWiredItems(IWiredItem Data, Habbo Habbo, int GroupId = 0)
        {
            List<Item> Items = new List<Item>();
            IWiredCycle Cycle = null;
            if (Items is IWiredCycle)
            {
                Cycle = (IWiredCycle)Items;
            }
            using (IQueryAdapter dbClient = HabboEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("INSERT INTO `wired_items` VALUES (@id, @items, @delay, @string, @bool)");
                dbClient.AddParameter("id", Data.Item.Id);
                dbClient.AddParameter("items", Items);
                dbClient.AddParameter("delay", (Data is IWiredCycle) ? Cycle.Delay : 0);
                dbClient.AddParameter("string", Data.StringData);
                dbClient.AddParameter("bool", Data.BoolData ? "1" : "0");


                int Item1Id = Convert.ToInt32(dbClient.InsertQuery());

                dbClient.SetQuery("INSERT INTO `items` (base_item,user_id,room_id,x,y,z,wall_pos,rot,extra_data) VALUES(@did,@uid,@rid,@x,@y,@z,@wallpos,@rot,@flags);");
                dbClient.AddParameter("did", Data.Item.Id);
                dbClient.AddParameter("uid", Habbo.Id);
                dbClient.AddParameter("rid", 0);
                dbClient.AddParameter("x", 0);
                dbClient.AddParameter("y", 0);
                ...
                Item Item1 = new Item(Item1Id, 0, Data.Item.Id, "", 0, 0, 0, 0, Habbo.Id, GroupId, 0, 0, "");
                Item Item2 = new Item(Item2Id, 0, Data.Item.Id, "", 0, 0, 0, 0, Habbo.Id, GroupId, 0, 0, "");

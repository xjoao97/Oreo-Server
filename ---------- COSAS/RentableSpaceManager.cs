...
namespace Quasar.HabboHotel.Items.RentableSpaces
{
    public class RentableSpaceManager
    {
        private static readonly ILog log = LogManager.GetLogger("Quasar.HabboHotel.Items.RentableSpaces");
        ...
        public void Init()
        {
            ...
            using (IQueryAdapter con = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (table != null)
                {
                    for (int i = 0; i < table.Rows.Count; i++)
                    {
                        ...
                        if (row != null)
                        {
                            ...
                            if (ownerid > 0)
                            {
                                Habbo owner = QuasarEnvironment.GetHabboById(ownerid);
                                if (owner != null)
        ...
        public bool ConfirmBuy(GameClient Session, RentableSpaceItem RentableSpace, int ExpireSeconds)
        {
            ...
            RentableSpace.ExpireStamp = (int)QuasarEnvironment.GetUnixTimestamp() + ExpireSeconds;
            ...
            using (IQueryAdapter con = QuasarEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                con.AddParameter("expire", QuasarEnvironment.GetUnixTimestamp() + 604800);
                con.RunQuery();
            }
            return true;

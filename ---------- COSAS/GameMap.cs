            ...
            if (mCoordinatedItems.ContainsKey(coord))
            {
                ...
                foreach (Item item in itemsFromSquare)
                {
                    if (item.GetBaseItem().InteractionType == InteractionType.RENTABLE_SPACE)
                        ...
                        if (QuasarEnvironment.GetGame().GetRentableSpaceManager().GetRentableSpaceItem(item.Id, out _rentableSpace))
                        {
                            ...
                            if (_rentableSpace.OwnerId == Session.GetHabbo().Id && QuasarEnvironment.GetUnixTimestamp() < _rentableSpace.ExpireStamp)
                            {                                                              

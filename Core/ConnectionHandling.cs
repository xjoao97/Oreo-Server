...
namespace Quasar.Core
{
    public class ConnectionHandling
    {
        ...
        private void manager_connectionEvent(ConnectionInformation connection)
        {
            ...
            QuasarEnvironment.GetGame().GetClientManager().CreateAndStartClient(Convert.ToInt32(connection.getConnectionID()), connection);
        }
        ...
        private void CloseConnection(ConnectionInformation Connection)
        {
            ...
            {
                Connection.Dispose();
                QuasarEnvironment.GetGame().GetClientManager().DisposeConnection(Convert.ToInt32( Connection.getConnectionID()));
            }
            catch (Exception e)
            {
